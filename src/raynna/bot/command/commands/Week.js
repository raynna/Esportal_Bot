    ...
    async execute(tags, channel, argument, client) {
        try {
            ...
            if (!gamesData || gamesData.length === 0) {
                ...
                return `${esportalName} has not played any games within 7 days. Last played: ${formattedDate} @${tags.username}`;
            }
            ...
            return `${esportalName}'s stats latest 7 days: Games: ${totalGames}, Rating: ${totalEloChange > 0 ? '+' : ''}${totalEloChange}, Kills: ${totalKills}, Deaths: ${totalDeaths}, K/D: ${ratio} @${tags.username}`;
        } catch (error) {
